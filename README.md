...
처음 실행하면 macOS의 입력 모니터링 권한을 허용해야 합니다. `시스템 설정 열기`를 누르면 작은 안내창이 함께 표시됩니다. 안내창의 `TypingPet.app` 아이콘을 입력 모니터링 목록으로 드래그하고 스위치를 켜세요. 드래그가 동작하지 않으면 목록 아래의 `+` 버튼으로 앱을 직접 선택할 수 있습니다.
